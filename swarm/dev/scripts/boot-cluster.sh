...
#
# The cluster contains a bootnode, a weth node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the weth and
# swarm nodes
start_bootnode() {
...
# start_weth_node starts a weth node with --datadir pointing at <base-dir>/weth
# and a single, unlocked account with password "weth"
start_weth_node() {
  local dir="${base_dir}/weth"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get weth account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${WETH_IP}"
    --rpcport   "${WETH_RPC_PORT}"
    --verbosity "6"
  )

  start_node "weth" "${WETH_IP}" "$(which weth)" ${args[@]}
}
...
create_account() {
  ...
  weth --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
