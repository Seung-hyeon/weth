...
func TestATFieldSigning(t *testing.T) {
	key, _ := crypto.GenerateKey()
	addr := crypto.PubkeyToAddress(key.PublicKey)

	signer := NewATFieldSigner(big.NewInt(18))
...
func TestATFieldChainId(t *testing.T) {
	key, _ := crypto.GenerateKey()
	addr := crypto.PubkeyToAddress(key.PublicKey)
...
func TestATFieldSigningVitalik(t *testing.T) {
	// Test vectors come from http://vitalik.ca/files/atfield_testvec.txt
	for i, test := range []struct {
		...
		signer := NewATFieldSigner(big.NewInt(1))
...
func TestChainId(t *testing.T) {
	...
	var err error
	tx, err = SignTx(tx, NewATFieldSigner(big.NewInt(1)), key)
	if err != nil {
		t.Fatal(err)
	}

	_, err = Sender(NewATFieldSigner(big.NewInt(2)), tx)
	if err != ErrInvalidChainId {
		t.Error("expected error:", ErrInvalidChainId)
	}

	_, err = Sender(NewATFieldSigner(big.NewInt(1)), tx)
	if err != nil {
