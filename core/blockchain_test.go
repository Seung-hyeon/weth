...
func TestFastVsFullChains(t *testing.T) {
	...
	var (
		...
		signer  = types.NewATFieldSigner(gspec.Config.ChainId)
	)
...
func TestATFieldTransition(t *testing.T) {
	// Configure and generate a sample block chain
	var (
		db, _      = ethdb.NewMemDatabase()
		key, _     = crypto.HexToECDSA("b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291")
		...
		gspec      = &Genesis{
			Config: &params.ChainConfig{ChainId: big.NewInt(1), ATFieldBlock: big.NewInt(2), HomesteadBlock: new(big.Int)},
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, db, 4, func(i int, block *BlockGen) {
		...
		case 2:
			...
			tx, err = basicTx(types.NewATFieldSigner(gspec.Config.ChainId))
			if err != nil {
				t.Fatal(err)
			}
			block.AddTx(tx)
	...
	config := &params.ChainConfig{ChainId: big.NewInt(2), ATFieldBlock: big.NewInt(2), HomesteadBlock: new(big.Int)}
	blocks, _ = GenerateChain(config, blocks[len(blocks)-1], db, 4, func(i int, block *BlockGen) {
		...
		switch i {
		case 0:
			tx, err = basicTx(types.NewATFieldSigner(big.NewInt(2)))
			if err != nil {
				t.Fatal(err)
			}
			block.AddTx(tx)
