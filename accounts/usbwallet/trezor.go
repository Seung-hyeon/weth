...
func (w *trezorDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, atfieldID *big.Int) (common.Address, *types.Transaction, error) {
	if w.device == nil {
		return common.Address{}, nil, accounts.ErrWalletClosed
	}
	return w.trezorSign(path, tx, atfieldID)
}
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, atfieldID *big.Int) (common.Address, *types.Transaction, error) {
	...
	if atfieldID != nil { // EIP-155 transaction, set chain ID explicitly (only 32 bit is supported!?)
		id := uint32(atfieldID.Int64())
		request.ATFieldId = &id
	}
