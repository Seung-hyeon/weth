...
import (
	...
	"github.com/EthereumVega/EVA-00D/internal/build"
)

var (
	// Files that end up in the weth*.zip archive.
	wethArchiveFiles = []string{
		"COPYING",
		executablePath("weth"),
	}

	// Files that end up in the weth-alltools*.zip archive.
...
func doArchive(cmdline []string) {
	var (
		...
		upload = flag.String("upload", "", `Destination to upload the archives (usually "wethstore/builds")`)
		ext    string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		...
		weth     = "weth-" + base + ext
		alltools = "weth-alltools-" + base + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(weth, wethArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{weth, alltools} {
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "weth-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "weth.exe" {
			wethTool = file
		} else {
	...
	// first section contains the weth binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Weth":     wethTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.weth.nsi", filepath.Join(*workdir, "weth.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("weth-" + archiveBasename(*arch, env) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "weth.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload = flag.String("upload", "", `Destination to upload the archive (usually "wethstore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	...
	build.MustRun(gomobileTool("bind", "--target", "android", "--javapkg", "org.ethereum", "-v", "github.com/EthereumVega/EVA-00D/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("weth.aar", filepath.Join(GOBIN, "weth.aar"))
		return
	}
	...
	archive := "weth-" + archiveBasename("android", env) + ".aar"
	os.Rename("weth.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "--target", "ios", "--tags", "ios", "-v", "github.com/EthereumVega/EVA-00D/mobile")
	...
	archive := "weth-" + archiveBasename("ios", env)
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "Weth.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "Weth.podspec", "--allow-warnings", "--verbose")
	}
}
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "wethstore/builds")`)
