...
func (ks *KeyStore) SignTx(account *Account, tx *Transaction, atfieldID *BigInt) (*Transaction, error) {
	if atfieldID == nil { // Null passed from mobile app
		atfieldID = new(BigInt)
	}
	signed, err := ks.keystore.SignTx(account.account, tx.tx, atfieldID.bigint)
	if err != nil {
		return nil, err
	}
...
func (ks *KeyStore) SignTxPassphrase(account *Account, passphrase string, tx *Transaction, atfieldID *BigInt) (*Transaction, error) {
	if atfieldID == nil { // Null passed from mobile app
		atfieldID = new(BigInt)
	}
	signed, err := ks.keystore.SignTxWithPassphrase(account.account, passphrase, tx.tx, atfieldID.bigint)
	if err != nil {
		return nil, err
	}
	return &Transaction{signed}, nil
